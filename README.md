...
라이브러리에 의존하지 않고 직접 구현함으로써 충돌 없이 안정적으로 동작하고, 줄바꿈 동작도 원하는 대로 제어하며, 하이라이팅 색상도 자유롭게 커스터마이징할 수 있었습니다.

---

## 사용한 라이브러리

**AvalonEdit** — 코드 에디터 컴포넌트. 줄 번호, 문법 하이라이팅, 자동 들여쓰기 등을 제공합니다.
WPF 기본 `TextBox`는 대용량 텍스트 편집에 성능 문제가 있어서 사용했습니다.

**System.Text.Json** — .NET 내장 JSON 직렬화 라이브러리. 브레인스토밍 레이아웃과 병합 이력 저장에 사용했습니다.

**System.Windows.Forms.FolderBrowserDialog** — WPF에는 폴더 선택 다이얼로그가 없어서 WinForms 것을 가져다 썼습니다.
`.csproj`에 `<UseWindowsForms>true</UseWindowsForms>`를 추가해서 사용합니다.